...
def train(model, train_dataset, train_loader, device, cfg, tokenizer):
    ...
    embedding_parameters = [
                {'params': [p for p in model.lstm_head.parameters()]},
                {'params': [p for p in model.mlp_head.parameters()]},
                {'params': [p for p in model.prompt_embeddings.parameters()]}
            ]
    # 设置优化器
    ...
    embedding_optimizer = transformers.AdamW(embedding_parameters, lr=cfg.lr, correct_bias=True)
    embedding_scheduler = transformers.get_linear_schedule_with_warmup(embedding_optimizer, num_warmup_steps=cfg.warmup_steps, num_training_steps=total_steps)
    ...
    for epoch in range(cfg.epochs):
        ...
        for batch_idx, sample in enumerate(train_loader):
            ...
            #print(info_embed.size())
            #print(ques_embed.size())
            #inputs, ques_embed, info_embed, start, end
...
def test(model, test_dataset, test_loader, device, cfg, tokenizer):
    with torch.no_grad():
        ...
        for batch_idx, sample in enumerate(test_loader):
            ...
            result[batch_idx] = []
            #ques = tokenizer.decode(ques_embed).replace(" ", "")
            #info = tokenizer.decode(info_embed).replace(" ", "")
